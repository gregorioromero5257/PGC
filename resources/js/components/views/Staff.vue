<template>
  <ais-instant-search class="staff container py-2" :search-client="searchClient" index-name="users">
    ...
    <div class="row" v-if="!loading && $gate.isActive()">
      ...
      <div class="col-md-12">
        <router-link to="/staff/0" class="btn btn-outline-orange float-right font-weight-bold" v-if="$gate.activeUser().role_id == 1 || $gate.activeUser().role_id == 2 || $gate.activeUser().role_id == 3">
    ...
    <div class="row mt-4">
      <div class="col-md-3 d-none d-md-block">
        <ais-refinement-list class="mb-3" attribute="status" searchable show-more :limit="5":show-more-limit="100">
          <ais-configure v-bind="initial"/>
          <div slot-scope="{items,isShowingMore,isFromSearch,canToggleShowMore,refine,createURL,toggleShowMore,searchForItems}">
            <p class="font-weight-bold mb-1">Estatus</p>
        ...
        <ais-refinement-list class="mb-3" attribute="position" searchable show-more :limit="10" :show-more-limit="100">
          <div slot-scope="{items,isShowingMore,isFromSearch,canToggleShowMore,refine,createURL,toggleShowMore,searchForItems}">
            <p class="font-weight-bold mb-1">Cargo</p>
      ...
      <div class="col-md-9">
        <ais-search-box>
          <div slot-scope="{ currentRefinement, isSearchStalled, refine }" class="form-group mb-0">
            <div class="input-group-prepend main-search">
              ...
              <input class="form-control search-input" type="text" v-model="query" @input="refine($event.currentTarget.value)" placeholder="Buscar colaborador...">
              <span class="input-group-text loading-icon" id="basic-addon1">
        ...
        <ais-stats>
          <p style="color: #495057; border-bottom:0.25rem solid #dee2e6;" class="mt-3 pb-3 h3" slot-scope="{ hitsPerPage, nbPages, nbHits }">
            {{ nbHits }} colaboradores encontrados
          </p>
        </ais-stats>

        <ais-current-refinements class="col-md-12">
          <div slot-scope="{ items }" class=mr-2>
            <span :it="it=items" v-for="item in items" :key="item.attribute">
          ...
          <div class="row mt-2 mb-4" slot-scope="{ items }" v-if="items.length > 0">
            <div class="col-md-12">
              <div class="d-none d-md-flex align-md-items-center flex-orientation table-head font-weight-bold">
                <div class="p-2 w-md-10"></div>
                <div class="p-2 w-md-30">Nombre</div>
                <div class="p-2 text-center w-md-25">Correo electrónico</div>
                <!-- <div class="p-2 text-center w-md-15">Rol</div> -->
                <div class="p-2 text-center w-md-15">Cargo</div>
                <div class="p-2 text-center w-md-20">Fecha de creación</div>
              </div>
              <div class="d-flex align-items-md-center flex-orientation table-row pointer" v-for="user in items" :key="user.objectID" @mouseover="optionsHover = user.objectID" @mouseleave="optionsHover = ''" @click="goToUser(user)">
                <div class="p-2 w-md-10">
                  <div v-if="user.avatar" class="avatar-bg m-0 d-block" :style="{ 'background': 'url(' + user.avatar + ')' }"></div>
                  <div v-else class="avatar-bg m-0 d-block" style="background-image: url('/img/user-circle-solid.svg');"></div>
                </div>
                <div class="p-2 pt-3 pb-3 w-md-30" :class="{ 'text-muted' : user.active == 0 }">
                  <span class="font-weight-bold d-md-none">Nombre: </span> <ais-highlight attribute="name" :hit="user"/>
                </div>
                <div class="p-2 pt-3 pb-3 text-md-center w-md-25" :class="{ 'text-muted' : user.active == 0 }">
                  <span class="font-weight-bold d-md-none">Correo electrónico: </span> {{ user.email }}
                </div>
                <!--                                 <div class="p-2 pt-3 pb-3 text-md-center w-md-15" :class="{ 'text-muted' : user.active == 0 }">
                <span class="font-weight-bold d-md-none">Rol: </span> <span v-if="user.role">{{ user.role }}</span>
              </div> -->
              <div class="p-2 pt-3 pb-3 text-md-center w-md-15" :class="{ 'text-muted' : user.active == 0 }">
                <span class="font-weight-bold d-md-none">Cargo: </span> <span v-if="user.position">{{ user.position }}</span>
              </div>
              <div class="p-2 pt-3 pb-3 text-md-center w-md-20" :class="{ 'd-md-none' : user.objectID == optionsHover, 'text-muted' : user.active == 0 }">
                <span class="font-weight-bold d-md-none">Fecha de creación: </span> {{ user.created_at | myDate }}
              </div>
              <div class="text-right-md d-md-none text-center w-md-20" :class="{ 'd-md-flex' : user.objectID == optionsHover }">
                <div class="btn-group w-100" role="group" aria-label="">
                  <toogle-status v-if="$gate.activeUser().role_id == 1 || $gate.activeUser().role_id == 2 || $gate.activeUser().role_id == 3" :user="user" @refreshStatus="refreshStatus" />
                  <a v-if="$gate.activeUser().role_id == 1 || $gate.activeUser().role_id == 2 || $gate.activeUser().role_id == 3" class="btn" :class="{ 'd-none' : user.active != 1 }" data-toggle="tooltip" data-placement="top" title="Editar colaborador" @click.stop="goToEditUser(user)">
                    <i class="fas fa-pencil-alt pointer"></i>
                  </a>
                  <a v-if="$gate.activeUser().role_id == 1 || $gate.activeUser().role_id == 2 || $gate.activeUser().role_id == 3" class="btn" :class="{ 'd-none' : user.active != 1 }" data-toggle="tooltip" data-placement="top" title="Eliminar colaborador" @click.stop="deleteUser(user)">
      ...
      <ais-pagination class="text-center" :class-names="{'ais-Pagination-list': 'pagination justify-content-center', 'ais-Pagination-item': 'page-item', 'ais-Pagination-link': 'page-link', 'ais-Pagination-item--selected': 'active', }"/>
    </div>
  </div>
...
export default {
  ...
  methods:{
    goToUser(user){
      this.$router.push(`/staff/${user.objectID}`)
    },
    goToEditUser(user){
      this.$router.push(`/staff/${user.objectID}`)
    },
    deleteUser(user){
      swal.fire({
        ...
        if (result.isConfirmed) {
          axios.get(`api/user-delete/${user.objectID}`).then(response => {
            this.getUsers();
            this.refreshStatus();
            this.query = '';
  ...
  created(){
    if(this.$gate.isActive()) this.getUsers();
  },
  mounted(){
    Fire.$on('AfterCreate',() => {
      this.getUsers();
