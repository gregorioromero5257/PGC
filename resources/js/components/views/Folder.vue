<template>
    <div class="folder container py-2" @click="closeDropdowns">
        <Loading v-if="loading" />
        <div class="row" v-if="!loading && $gate.isActive()">
            <div class="col-md-12">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item active" aria-current="page" v-if="!folder.upperfolder_id"><router-link class="text-orange" to="/resources">Recursos</router-link></li>
                        <li class="breadcrumb-item active" aria-current="page" v-else><a class="text-orange pointer" @click="goToUpperFolder">{{ folder.folder.name }}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ folder.name }}</li>
                    </ol>
                </nav>
            </div>
            <div class="col-md-12 mb-4">
                <div v-if="$gate.isActive()" class="dropdown">
                    <a href="#" class="btn btn-outline-orange float-right font-weight-bold pointer" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <i class="fas fa-plus mr-1"></i>
                        Nuevo
                    </a>
                    <div class="dropdown-menu dropdown-menu-right mt-2" aria-labelledby="dropdownMenuLink">
                        <a class="dropdown-item" href="#" @click.stop="addFolderModal"><i class="fas fa-folder-plus mr-2"></i>Carpeta</a>
                        <a class="dropdown-item" href="#" @click.stop="addResourceModal"><i class="fas fa-file-upload mr-2"></i> Subir archivo</a>
                    </div>
                </div>
            </div>
            <div class="col-md-12 mb-1 d-flex justify-content-between align-items-center">
                ...
                <div v-if="$gate.isActive()" class="float-right" v-show="folderSelected">
            ...
            <div class="col-md-12 mb-4">
                ...
                <!-- ************************************* file lock when downloading ************************************-->
                <center>
                  <Downloading v-if="loadingDowload" />
                </center>
            </div>
            <div class="col-md-12" v-show="folder.folders && folder.folders.length > 0">
                <p class="text-secondary">Carpetas</p>
            </div>
            <div class="col-md-3 pointer" v-for="subfolder in folder.folders" :key="subfolder.id">
<!--                 <div :class="{ 'card-active' : folderSelected == subfolder }" class="card card-shadow bg-light mb-3" @dblclick.stop="goToFolder(subfolder)" @click.stop="selectFolder(subfolder)" @contextmenu.prevent.stop="folderOptions(subfolder)">
 -->
                 <div :class="{ 'card-active' : folderSelected == subfolder }" class="card card-shadow bg-light mb-3" @click="goToFolder(subfolder)"  @contextmenu.prevent.stop="folderOptions(subfolder)">
                    <div class="card-body">
                        <div class="media">
                            <i class="fas fa-folder fa-lg align-self-center mr-2"></i>
                            <div class="media-body align-self-center">
                                <p class="mb-0">
                                    {{ subfolder.name }}
                                </p>
                            </div>
                            <button type="button" class="btn btn-options d-md-none align-self-center" @click.stop="folderOptions(subfolder)">
                                <i class="fas fa-ellipsis-v"></i>
                            </button>
                        </div>
                    </div>
                </div>
                <div :id="'dropdown'+subfolder.id" class="dropdown-menu dropdown-menu-right-folder" aria-labelledby="dropdownMenuButton">
                    <a v-if="!$gate.isUser()" class="dropdown-item" href="#" @click.stop="editFolderModal(subfolder)">
                        <i class="fas fa-pencil-alt pointer mr-2"></i> Cambiar nombre
                    </a>
                    <a v-if="!$gate.isUser()" class="dropdown-item" href="#" @click.stop="deleteFolder(subfolder)">
                        <i class="far fa-trash-alt pointer mr-2"></i> Eliminar
                    </a>
                </div>
            </div>
            <div class="col-md-12" v-show="folder.resources && folder.resources.length > 0" v-if="!loadingDowload">
                <p class="text-secondary">Archivos</p>
            </div>
            <div class="col-md-3 pointer" v-if="!loadingDowload" v-for="resource in folder.resources" :key="resource.id">
<!--                 <div :class="{ 'card-active' : resourceSelected == resource }" class="card card-shadow bg-light mb-3" @dblclick.stop="downloadResource(resource)" @click.stop="selectResource(resource)" @contextmenu.prevent.stop="resourceOptions(resource)">
 -->
                <div :class="{ 'card-active' : resourceSelected == resource }"  class="card card-shadow bg-light mb-3" @click="downloadResource(resource)"  @contextmenu.prevent.stop="resourceOptions(resource)">
                    <div class="card-body">
                        <div class="media">
                            <i
...
<script>
    ...
    import Downloading from '../complements/Downloading'
    ...
    export default {
        ...
        methods:{
            ...
            deleteResource(resource){
                ...
                swal.fire({
                    ...
                    if (result.value) {
                        axios.delete(`/api/resource/${resource.id}`)
                        .then(()=>{
            ...
            downloadResource(resource){
              console.log(resource);
                ...
                axios({
                    ...
                    link.href = url;
                    link.setAttribute('download', resource.name + resource.type);
            ...
            resourceOptions(resource){
                $(".dropdown-menu").dropdown('hide');
                this.resourceSelected = resource;
                $('#dropdown'+resource.id).dropdown('show')
            },
            ...
            goToUpperFolder(){
                this.$router.push({ name: 'folder', params: { id: this.folder.upperfolder_id } });
                Fire.$emit('AfterCreate');
            },
            deleteFolder(folder){
                swal.fire({
            ...
            addResourceModal(){
                ...
                this.$refs.AddResource.form.reset();
                $(".dropdown-menu").dropdown('hide');
            },
            getFolder(){
                this.loading = true;
                axios.get(`/api/folder/${this.$route.params.id}`)
                .then(
                    ({data}) => (
                        this.folder = data,
                        this.loading = false
        ...
        computed: {
            hasResources: function () {
                if((this.folder.folders && this.folder.folders.length) || (this.folder.resources && this.folder.resources.length)) {
                    return true
                } else {
                    return false
                }
