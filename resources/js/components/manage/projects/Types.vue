            ...
            <div class="col-md-10">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Tipos de proyectos</li>
                    </ol>
                </nav>
            </div>
            <div class="col-md-10">
                <h1 class="h2 text-center font-weight-bold">Tipos de Proyectos</h1>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <tr v-for="type in types" :key="type.id">
                                <td width="60%">{{ type.name }}</td>
                                <td class="text-center">{{ type.created_at | myDate }}</td>
                                <td class="text-center">
                                    <a class="text-dark pointer" @click="editModal(type)">
                                        <i class="fas fa-pencil-alt pointer" data-toggle="tooltip" data-placement="top" title="Editar"></i>
                                    </a> 
                                </td>
                                <td class="text-center">
                                    <a class="text-dark pointer" @click="deleteType(type)">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="ModalTitle" v-show="!editmode">Agregar tipo de proyecto</h5>
                        <h5 class="modal-title" id="ModalTitle" v-show="editmode">Editar tipo de proyecto</h5>
        ...
        methods: {
            ...
            createType(){
                ...
                .then((response)=>{
                    toast.fire({
                        type: 'success',
                        title: `Tipo de proyecto agregado correctamente.`
                    })
            ...
            updateType(){
                ...
                .then(() => {
                    ...
                    toast.fire({
                        type: 'success',
                        title: `Tipo de proyecto editado correctamente.`
                    })
                    this.$Progress.finish();
                    Fire.$emit('AfterCreate');
                }).catch(() => {
            ...
            deleteType(type){
                swal.fire({
                    ...
                    if (result.value) {
                        this.$Progress.start();
                        axios.delete(`/api/type/${type.id}`).then(()=>{
                            swal.fire(
                                'Eliminado',
                                'Tipo de proyecto ha sido eliminado.',
                                'success'
                            )
                            Fire.$emit('AfterCreate');
                            this.$Progress.finish();
