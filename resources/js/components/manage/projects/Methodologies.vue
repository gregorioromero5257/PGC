            ...
            <div class="col-md-10">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Análisis, metodologías e implementación</li>
                    </ol>
                </nav>
            </div>
            <div class="col-md-10">
                <h1 class="h2 text-center font-weight-bold">Análisis, metodologías e implementación</h1>
            ...
            <div class="col-md-10 mt-3">
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <tr v-for="methodology in methodologies" :key="methodology.id">
                                <td>{{ methodology.name }}</td>
                                <td class="text-center">{{ methodology.created_at | myDate }}</td>
                                <td class="text-center">
                                    <a class="text-dark pointer" @click="editModal(methodology)">
                                        <i class="fas fa-pencil-alt pointer" data-toggle="tooltip" data-placement="top" title="Editar"></i>
                                    </a> 
                                </td>
                                <td class="text-center">
                                    <a class="text-dark pointer" @click="deleteMethodology(methodology)">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form @submit.prevent="!editmode ? createMethodology() : updateMethodology()">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            createMethodology(){
                this.$Progress.start();
                this.form.post('/api/methodology')
                .then((response)=>{
                    toast.fire({
                        type: 'success',
            ...
            editModal(methodology){
                this.editmode = true;
                this.form.reset();
                $('#Modal').modal('show');
                this.form.fill(methodology);
            },
            updateMethodology(){
                this.$Progress.start();
                this.form.put('/api/methodology/'+this.form.id)
                .then(() => {
                    $('#Modal').modal('hide');
                    toast.fire({
                        type: 'success',
            ...
            deleteMethodology(methodology){
                swal.fire({
                    ...
                    if (result.value) {
                        this.$Progress.start();
                        axios.delete(`/api/methodology/${methodology.id}`).then(()=>{
                            swal.fire(
                                'Eliminado',
