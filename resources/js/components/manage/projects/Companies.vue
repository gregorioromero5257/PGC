            ...
            <div class="col-md-10">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Empresas y oficinas</li>
                    </ol>
                </nav>
            </div>
            <div class="col-md-10">
                <h1 class="h2 text-center font-weight-bold">Empresas y Oficinas</h1>
                <button type="button" class="btn btn-outline-orange float-right font-weight-bold pointer" @click="newModal">
                    <i class="fas fa-plus mr-1"></i>
                    Agregar empresa u oficina
                </button>
            </div>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <tr v-for="company in companies" :key="company.id">
                                ...
                                <td>
                                    <a class="text-dark pointer" @click="editModal(company)">
                                        <i class="fas fa-pencil-alt pointer" data-toggle="tooltip" data-placement="top" title="Editar"></i>
                                    </a> 
                                </td>
                                <td>
                                    <a class="text-dark pointer" @click="deleteCompany(company)">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="ModalTitle" v-show="!editmode">Agregar empresa u oficina</h5>
                        <h5 class="modal-title" id="ModalTitle" v-show="editmode">Editar empresa u oficina</h5>
        ...
        methods: {
            ...
            createCompany(){
                ...
                .then((response)=>{
                    toast.fire({
                        type: 'success',
                        title: `Empresa u oficina agregada correctamente.`
                    })
            ...
            updateCompany(company){
                ...
                .then(() => {
                    ...
                    toast.fire({
                        type: 'success',
                        title: `Empresa u oficina editada correctamente.`
                    })
                    this.$Progress.finish();
                    Fire.$emit('AfterCreate');
                }).catch(() => {
            ...
            deleteCompany(){
                swal.fire({
                    ...
                    if (result.value) {
                        this.$Progress.start();
                        axios.delete(`/api/company/${company.id}`).then(()=>{
                            swal.fire(
                                'Eliminado',
                                'La empresa u oficina ha sido eliminada.',
                                'success'
                            )
                            Fire.$emit('AfterCreate');
                            this.$Progress.finish();
